...
function ExperienceItem({ title, date, description }) {
  return (
    <div className="md:mx-0 block w-64 py-6 px-8 mx-auto bg-cardBG text-greyText rounded-[20px] shadow-[0px_0_10px_rgba(0,0,0,0.8)]">
      <h3 className="text-lg font-medium text-pinkText md:text-xl">{title}</h3>
      <p className="my-2 text-blueText">
        <em>{date}</em>
      </p>
      <p>{description}</p>
    </div>
  ...
  return (
    <>
      ...
      <header className="pb-8 mx-auto font-bold text-center">
        <h1>
          <MovingText text="Mine erfaringer" textSize="text-xl md:text-3xl" />
        </h1>
        <h2>
          <MovingText
      ...
      <main className="max-w-6xl mx-auto text-greyText md:text-lg">
        <section>
          <h2 className="mt-2 text-xl text-center md:text-2xl text-[#000] font-bold mb-5">
            Arbeidserfaring
          </h2>
        ...
        <section className="mt-16">
          ...
          <div className="flex flex-wrap justify-center gap-16">
            {courses.map((course, index) => (
              <ExperienceItem key={index} {...course} />
            ))}
          </div>
        </section>
      </main>
